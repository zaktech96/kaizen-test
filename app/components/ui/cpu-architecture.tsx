...
export const CpuArchitecture = ({
  ...
  return (
    ...
    >
      ...
      {nodeLabels.length > 0 && (
        <g fontSize="5" fontWeight="500" fill="currentColor">
          {nodeLabels.slice(0, 8).map((label, idx) => {
            const coords = [
              { x: 8, y: 18, anchor: "end" }, // path 1 (left top)
              { x: 192, y: 15, anchor: "start" }, // path 2 (right top)
              { x: 142, y: 38, anchor: "start" }, // path 3 (upper right mid)
              { x: 178, y: 80, anchor: "start" }, // path 4 (right bottom)
              { x: 162, y: 67, anchor: "start" }, // path 5 (right lower mid)
              { x: 100, y: 98, anchor: "middle" }, // path 6 (bottom center)
              { x: 70, y: 88, anchor: "end" }, // path 7 (left bottom)
              { x: 28, y: 32, anchor: "end" }, // path 8 (left mid)
            ][idx];
            return coords ? (
              <text
                key={idx}
